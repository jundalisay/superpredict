<script lang="ts">
	...
	let logs = $state<ArticleLogEntry[]>([]);
	...
	async function load() {
		loading = true;
		try {
			const res = await fetch(`/api/data?country=ph&days=${days}`);
	...
	async function triggerAnalysis() {
		...
		try {
			const res = await fetch('/api/trigger', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ country: 'ph' })
			});
			const data = await res.json();
			if (data.success) {
				runResult = `Done! Worker: ${data.results.ph?.worker?.toFixed(1) ?? 0}, Warrior: ${data.results.ph?.warrior?.toFixed(1) ?? 0}, Thinker: ${data.results.ph?.thinker?.toFixed(1) ?? 0}, Trader: ${data.results.ph?.trader?.toFixed(1) ?? 0} (${data.results.ph?.articleCount ?? 0} articles)`;
				await load();
			} else {
...
<svelte:head>
	<title>PH Dashboard — Supersociology Predict</title>
</svelte:head>
...
<div class="page">
	<div class="page-header">
		<div>
			...
			<p class="page-sub">Daily sociological pulse from Philippine news sources</p>
		</div>
	...
	<div class="info-bar">
		<span>Source: Rappler.com</span>
		<span>Scheduled: midnight Philippine time</span>
	...
	{#if loading}
		<div class="loading">
			...
			<span>Loading PH data…</span>
		</div>
	{:else}
		<ScoreCards {rows} />
		<LineChart {rows} title="PH — Sociological Pulse" />

		<div class="source-card">
			<h3 class="source-title">About Rappler</h3>
			<p class="source-text">
				Rappler is a Philippine-based digital news organization known for in-depth reporting
				on politics, business, lifestyle, and social issues. Founded in 2012, it covers
				the full spectrum of Philippine public life — from Malacañang to the streets.
				All Philippine archetype scoring is derived from its latest headlines.
			</p>
		</div>
